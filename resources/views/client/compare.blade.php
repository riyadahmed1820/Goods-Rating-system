    		...
    		<div class="row justify-content-center">
    			<div class="col-md-10 mb-5 text-center">
    				<ul class="product-category">
    					<li><a href="{{URL::to('/compare')}}" class="{{(request()->is('shop')?'active':'')}}" >All</a></li>
                        @foreach($locations as $location)
    					<li><a href="/view_by_loc/{{$location->id}}" class="{{(request()->is('/view_by_loc/'.$location->location_name)?'active':'')}}">{{$location->location_name}}</a></li>
                        @endforeach
    				</ul>
    			</div>
    		</div>
